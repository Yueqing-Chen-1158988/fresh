...
def populate_data():
    ...
    # Add some customers
    customers = [
        Customer(name="John Doe", email="john@example.com", balance=0.0),
        Customer(name="Jane Smith", email="jane@example.com", balance=10.0)
    ]

    # Add some corporate customers
    corporate_customers = [
        CorporateCustomer(name="TechCorp", email="corp@tech.com", balance=500.0, credit_limit=1000.0, discount_rate=0.15)
    ]

    # Add all data to the session
    session.add_all(vegetables + premade_boxes + customers + corporate_customers)

    # Commit initial data
    session.commit()

    # Retrieve some customer and product records for order creation
    john_doe = session.query(Customer).filter_by(name="John Doe").first()
    carrot = session.query(Vegetable).filter_by(name="Carrot").first()
    small_box = session.query(PremadeBox).filter_by(size="small").first()

    # Create an order for John Doe
    order_john = Order(
        customer_id=john_doe.customer_id, 
    ...
    order_line_1 = OrderLine(
        order_id=order_john.order_id,  # This will be set after the order is added to the session
    ...
    # Create a payment for John Doe's order
    payment_john = Payment(
        order_id=order_john.order_id,  # This will be set after the order is added to the session
    ...
    session.add(order_john)  # Add the order first
    session.add(order_line_1)
    session.add(order_line_2)
    session.add(payment_john)
